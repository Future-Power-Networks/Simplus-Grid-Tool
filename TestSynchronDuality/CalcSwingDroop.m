...
function [Z_FD,Ztot_prime,Sit,Sit_prime,root,root_prime,pole_sys,Gv_cl,Gi_cl] = CalcSwingDroop(varargin)
...
%%
% Update values according to varargin
for i = 1:length(varargin)
    if(strcmpi(varargin{i},'Xline'))
        Xline = varargin{i+1};
    elseif (strcmpi(varargin{i},'Rline'))
        Rline = varargin{i+1};
    elseif (strcmpi(varargin{i},'m'))
        m = varargin{i+1}; 
    elseif (strcmpi(varargin{i},'w_f'))
        w_f = varargin{i+1};
    elseif (strcmpi(varargin{i},'w_v'))
        w_v = varargin{i+1};
    elseif (strcmpi(varargin{i},'V'))
        V = varargin{i+1};
    elseif (strcmpi(varargin{i},'I'))
        I = varargin{i+1};
    elseif (strcmpi(varargin{i},'EnableInnerLoop'))
        EnableInnerLoop = varargin{i+1};
    end
end
