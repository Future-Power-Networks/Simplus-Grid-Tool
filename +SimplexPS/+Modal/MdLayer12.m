...
for k = 1:N_Bus
   if (ismember(k,DeviceSel)) %if selected 
       ...
       Layer2.real(Count) = real(Layer2All(k))/Layer2RealSum;
       Layer2.imag(Count) = imag(Layer2All(k))/Layer2ImagSum;
...
set(gca,'XTickLabel',[]);
title ('Impedance Participation Level-2 Real (Normalized)');
% for i=1:Count
...
b.FaceColor = 'flat';
b.CData = [1,0.5,0];
title ('Impedance Participation Level-2 Imag (Normalized)');
% for i=1:Count
