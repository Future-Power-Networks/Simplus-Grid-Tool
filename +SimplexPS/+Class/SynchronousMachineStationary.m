...
classdef SynchronousMachineStationary < SimplexPS.Class.SynchronousMachine
    ...
    methods(Static)
        ...
        function [Output] = StateSpaceEqu(obj,x,u,CallFlag)
            ...
            % ??? Temp
            psi_f = obj.psi_f;
            v_ab = SimplexPS.dq2alphabeta([v_d;v_q],theta);
            i_dq = SimplexPS.alphabeta2dq([i_a;i_b],theta);
            v_a  = v_ab(1);
            v_b  = v_ab(2);
            i_d  = i_dq(1);
            i_q  = i_dq(2);   
    ...
    methods(Access = protected)
        function resetImpl(obj)
            if obj.EquiInitial
                i_dq = obj.x_e(1:2);
                theta = obj.x_e(end);
                i_ab = SimplexPS.dq2alphabeta(i_dq,theta);
