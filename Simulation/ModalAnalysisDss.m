...
function [PfAllValue,OutStateStr,TargetMode]= ModalAnalysisDss(ObjGsysSs,ModeIndex,PfNumTh,FigN,EnablePlot)
    ...
    for i =1:length(ModeIndex)
        ...
        for PfMaxNum = 1:length(PfVec)
            ...
            % value.
            if (PfRestValue/PfVecSum<=0.01) || (PfMaxNum>=PfNumTh) 
                break;
            end
        end
        fprintf('\n')
        ...
        for k = 1:length(PfMaxIndex)
            ...
            PfStateStr_ = strrep(GsysSsStateStr{PfMaxIndex(k)},'_','\_');
            PfStateStr_ = strrep(PfStateStr_,'epsilon','\theta');
            PfStateStr_ = strrep(PfStateStr_,'w','\omega');
            PfStateStr{k} = PfStateStr_;
        end
