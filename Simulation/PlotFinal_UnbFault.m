...
%% Plot
% TimeShift = 2;      % For high inertia
TimeShift = 8;      % For med and low inertia test only

% FigSize = [0.1 0.1 0.35 0.7];
FigSize = [0.1 0.1 0.25 0.6];

% load(['out_ABG_Fault_' DataName]);
load(['out_AG_Fault_' DataName]);
...
if Enable_SaveFigure
    print(gcf,['Simulation\UnbFault_' DataName '.png'],'-dpng','-r600');
end