...
classdef SynchronousMachine < SimplusGT.Class.ModelAdvance
    ...
    methods(Static)
        ...
        function [x_e,u_e,xi] = Equilibrium(obj)
            ...
            i_abc = SimplusGT.dq2abc([i_d;i_q],xi);
            i_a = i_abc(1);
            i_b = i_abc(2);
            i_c = i_abc(3);
        ...
        function [Output] = StateSpaceEqu(obj,x,u,CallFlag)
            ...
            %i_dq1 = sqrt(2/3)*[cos(theta), cos(theta-2/3*pi), cos(theta-4/3*pi);
                   %        -sin(theta),-sin(theta-2/3*pi),-sin(theta-4/3*pi);
                   %       1/sqrt(2),  1/sqrt(2),         1/sqrt(2)]*[i_a;i_b;i_c];
            i_dq1 =  SimplusGT.abc2dq(i_abc,theta);       
            i_d = i_dq1(1);
            i_q = i_dq1(2);
            ...
            v_d  =  u(1); 
            v_q  =  u(2);   
            v_abc = SimplusGT.dq2abc([v_d; v_q;],theta);
            T_m  = u(3);
            v_ex = u(4);
            ...
            % ### Call state equation: dx/dt = f(x,u)
                ...
                %i_abc = SimplexPS.dq2abc([i_d;i_q;],theta);
                psi_f = obj.psi_f;
                %psi_d = L*i_d;
                %psi_q = L*i_q - psi_f;
                if obj.ApparatusType == 0
                    Te = psi_f*sqrt(2/3)*[cos(theta), cos(theta-2*pi/3), cos(theta-4*pi/3)]*i_abc;
              
                elseif obj.DeviceType == 1
                    Pe = psi_f*W0*sqrt(2/3)*[cos(theta), cos(theta-2*pi/3), cos(theta-4*pi/3)]*i_abc;      % Means e_d = psi_f*W0 is constant.
                else
                    error(['Error.']);
                end
                ...
                di_abc  = (v_abc - R*i_abc - w*sqrt(2/3)*[cos(theta)*psi_f;
                                                cos(theta-2*pi/3)*psi_f;
                                                cos(theta-4*pi/3)*psi_f])/L;
                %di_dq = T*di_abc + dT*i_abc;
                %di_d = di_dq(1);
                %di_q = di_dq(2);
                %di_0   = 0;
                dw     = (Te - T_m - D*w)/J;
