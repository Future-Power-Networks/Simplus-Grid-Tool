...
Enable_Plot_GraphOrigin         = 0;    % 1/0: Plot the graph of the original power system
...
GbusVI  = Gbus;
GbusVIF = HybridMatrixYZ(YbusVIF,n_Ibus_1st);
...
% Get K matrix (synchronizing coefficient)
for m = 1:N_Bus
    for n = 1:N_Bus
        if n ~= m
...
% Get Gamma matrix (damping coefficient)
for m = 1:N_Bus
    for n = 1:N_Bus
